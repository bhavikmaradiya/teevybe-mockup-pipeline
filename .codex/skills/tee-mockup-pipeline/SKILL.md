...
4. For every web/cloud Scheduled task, read [references/cloud-size-charts.md](references/cloud-size-charts.md) before the capability gate and size-chart handling. It defines the native Drive-copy route for the unchanged approved `07.jpg`; local/project runs continue to use and hash the repository asset directly.
5. When a cloud or scheduled run is configured to discover source artwork from Google Drive, read [references/drive-intake.md](references/drive-intake.md) before listing or downloading intake files. Treat intake as read-only and require its separate Drive-backed durable claim ledger before generation.
6. When Drive mirroring is configured for the batch, also read [references/drive-publish.md](references/drive-publish.md) before the publish stage.
...
- Operations QA must not generate images, interpret artwork, accept identities, draw a size chart, alter T-shirt artwork, or choose logo placement. For local/project `07.jpg`, it may only byte-copy the approved profile-matched final template and report hashes and dimensions. For a web/cloud Scheduled task, it may instead perform only the profile-matched native Drive copies and readback checks authorized by [references/cloud-size-charts.md](references/cloud-size-charts.md).
